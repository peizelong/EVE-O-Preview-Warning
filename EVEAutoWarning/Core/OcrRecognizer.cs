...
public class OcrRecognizer : IDisposable
{
        ...
        try
        {
            ...
            if (Directory.Exists(tessPath))
            {
                ...
                if (File.Exists(langFile))
                {
                    ...
                    _initialized = true;
                    Console.WriteLine($"[OCR] Tesseract 初始化成功，语言: {_language}");
                }
                else
                {
                    Console.WriteLine($"[OCR] 未找到语言文件: {langFile}");
                    Console.WriteLine("[OCR] 请下载 tessdata 文件并放置在程序目录下的 tessdata 文件夹中");
                    Console.WriteLine("[OCR] 下载地址: https://github.com/tesseract-ocr/tessdata");
                    _initialized = false;
                }
            }
            else
            {
                Console.WriteLine($"[OCR] tessdata 目录不存在: {tessPath}");
                Console.WriteLine("[OCR] 请创建 tessdata 文件夹并放入语言文件");
                _initialized = false;
            }
        }
        catch (Exception ex)
    ...
    {
        if (!_initialized || _engine == null || bitmap == null)
        {
            return string.Empty;
        }
        ...
        return await Task.Run(() =>
        {
            try
            {
                ...
                try
                {
                    regionBitmap.Save(tempFile, System.Drawing.Imaging.ImageFormat.Png);
    ...
    public async Task<List<OcrTextResult>> RecognizeTextsWithPositionsAsync(Bitmap bitmap, Rectangle searchRegion)
    {
        ...
        await Task.Run(() =>
        {
            ...
            {
                Console.WriteLine($"[OCR] 批量识别失败: {ex.Message}");
            }
        });
    ...
    private Bitmap CropBitmap(Bitmap source, Rectangle region)
    {
        ...
        {
            g.DrawImage(source, new Rectangle(0, 0, region.Width, region.Height),
