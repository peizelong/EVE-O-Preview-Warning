...
## New: Per-Client Template Alerting (WGC + ROI)

Each EVE client thumbnail can be monitored independently for danger (red / orange / white flag names). The monitor:

- Captures a per-window frame via `WgcCaptureService` (PrintWindow API, WGC-compatible interface).
- Runs red / orange / white list template matching in a user-selected ROI.
- Plays an alarm sound (configurable path, default Windows Exclamation) on rising edge.
- Renders a red border on the thumbnail for the duration of the alert.
- Uses hysteresis: `AlertConfirmationFrames=2` (rising), `AlertClearFrames=4` + `MinAlertDurationMs=2000` (falling) to prevent flicker.

Configuration keys in `EVE-O-Preview.json`:

```json
{
  "EnableWgcCapture": true,
  "EnableTemplateDetection": true,
  "TemplateMatchThreshold": 0.85,
  "TemplateScanIntervalMs": 500,
  "AlarmSoundPath": "",
  "AlertConfirmationFrames": 2,
  "AlertClearFrames": 4,
  "MinAlertDurationMs": 2000,
  "PerClientRoi": { "EVE - ToonName": { "X": 100, "Y": 100, "Width": 400, "Height": 200 } }
}
```

To select an ROI for a client, choose *Select ROI* on the thumbnail context menu (UI hook provided by `IThumbnailManager.RequestRoiSelection`).
