...
namespace EveOPreview.Services
{
	sealed class ThumbnailManager : IThumbnailManager
	{
		...
		public ThumbnailManager(IMediator mediator, IThumbnailConfiguration configuration, IProcessMonitor processMonitor, IWindowManager windowManager, IThumbnailViewFactory factory, IWindowCaptureService wgc, ImageTemplateDetector detector)
		{
			...
			this._monitors = new Dictionary<IntPtr, ThumbnailWarningMonitor>();
		...
		public void Stop()
		{
			...
			foreach (var m in this._monitors.Values)
			{
				m.Stop();
				m.Dispose();
			}
		...
		public void StartDetection()
		{
			...
			// 为现有视图启动检测
			foreach (var kv in _thumbnailViews)
			{
				if (!_monitors.ContainsKey(kv.Key) && _configuration.EnableTemplateDetection)
				{
					var monitor = new ThumbnailWarningMonitor(
						_configuration, _wgc, _detector, _mediator,
						kv.Value.Title, kv.Key);
					_monitors[kv.Key] = monitor;
					monitor.Start();
				}
			}
		}
		...
		public bool IsDetectionRunning => _isDetectionActive;

		public Rectangle? RequestRoiSelection(string clientTitle)
		{
			var view = _thumbnailViews.Values.FirstOrDefault(v => v.Title == clientTitle);
			IntPtr hwnd = view?.Id ?? IntPtr.Zero;
			using var f = new RoiSelectorForm(hwnd);
			if (f.ShowDialog() == DialogResult.OK)
			{
				return f.SelectedRegion;
			}
			return null;
		}
		...
		private async void UpdateThumbnailsList()
		{
			...
			foreach (IProcessInfo process in addedProcesses)
			{
				...
				// 启动该 thumbnail 的报警 monitor（仅当检测已激活时）
				if (this._isDetectionActive && this._configuration.EnableTemplateDetection && !this._monitors.ContainsKey(view.Id))
				{
					var monitor = new ThumbnailWarningMonitor(
						this._configuration, this._wgc, this._detector, this._mediator,
						view.Title, view.Id);
			...
			foreach (IProcessInfo process in removedProcesses)
			{
				...
				// 停止该 thumbnail 的报警 monitor
				if (this._monitors.TryGetValue(view.Id, out var monitor))
				{
					monitor.Stop();
					monitor.Dispose();
